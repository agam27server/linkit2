...
/**
 * Generate a simple, clean, black-and-white QR code (with optional center logo)
 * @param {string} url - The URL to encode in the QR code
 * @returns {Promise<string>} - Base64 data URL of QR code PNG
 */
export const generateQRCode = async (url) => {
  try {
    console.log("QR Helper: Generating simple QR for:", url);
...
export const generateProfileQRCode = async (username, baseUrl) => {
  try {
    ...
    return await generateQRCode(profileUrl);
  } catch (error) {
    console.error("QR Helper: Error generating profile QR:", error);
    throw error;
  }
};
